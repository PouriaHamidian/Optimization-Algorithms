...
function [GlobalCost,BestNetWork]=JADE(net,Inputs,Targets,hiddenLayerSize,run,~,~,~)
...
bounds= 10*[-ones(pop_size,1), ones(pop_size,1)];
max_nfes = M * pop_size;
...
%     parfor run_id = 1 : runs
        ...
        for brojac = 1:size(pop,1)
            fitness(brojac) = feval(CostFunction, pop(brojac,:));
        end
%         fitness = feval(fhd,pop',func);
        ...
        while nfes < max_nfes
            ...
            for brojac = 1:size(ui,1)
                children_fitness(brojac) = feval(CostFunction, ui(brojac,:));
            end
...
fprintf('LSHADE_cnEpSin, NumNeuron: %d, run:%d, bestR2:%3.5f \n',hiddenLayerSize,run,GlobalCost);
