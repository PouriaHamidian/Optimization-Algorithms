...
function [GlobalCost,BestNetWork]=UMOEA(net,Inputs,Targets,hiddenLayerSize,run)
...
while stop_con==0
    ...
    if (current_eval<Par.Max_FES)
        if rand<Probs(1)
            ...
            [EA_1, EA_obj1,probDE1,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,current_eval,res_det] = ...
                SAMO_DE(CostFunction, EA_1, EA_obj1,probDE1,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,...
    ...
    if (current_eval<Par.Max_FES)
        if   rand<Probs(2)
            [ EA_2, EA_obj2, setting,bestold,bestx,bnd,fitness,current_eval,res_det] = ...
                SAMO_ES(CostFunction, EA_2, EA_obj2, setting, iter,bestold,bestx,fitness,bnd,...
    ...
    if current_eval>0.75*Par.Max_FES
        if rand<Par.prob_ls
            ...
            [bestx,bestold,current_eval,succ] = LS (CostFunction, bestx,bestold,Par,current_eval,Par.Max_FES,Par.xmin,Par.xmax);
...
GlobalCost=-bestold;
fprintf('UMOEA, NumNeuron: %d, run:%d, bestR2:%3.5f \n',hiddenLayerSize,run,GlobalCost);
end
