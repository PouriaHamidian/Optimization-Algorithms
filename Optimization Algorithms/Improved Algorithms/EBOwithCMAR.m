...
function [GlobalCost,BestNetWork]=JADE(net,Inputs,Targets,hiddenLayerSize,runn,~,~,~)
...
for brojac = 1:size(x,1)
    fitx(brojac) = CostFunction(x(brojac,:));
end
    ...
    if (current_eval<Par.Max_FES)
        if rand<Probs(1)
            ...
            [EA_1, EA_1old, EA_obj1,probDE1,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq, current_eval,res_det] = ...
                EBO(CostFunction, EA_1,EA_1old, EA_obj1,probDE1,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,....
    ...
    if (current_eval<Par.Max_FES)
        if   rand<Probs(2)
            [ EA_2, EA_obj2, setting,bestold,bestx,bnd,fitness,current_eval,res_det] = ...
                Scout(CostFunction, EA_2, EA_obj2, probSC, setting, iter,bestold,bestx,fitness,bnd,...
...
%             disp([min(EA_obj2) Probs(1) Probs(2)]);
    ...
    if current_eval>0.75*Par.Max_FES
        if rand<Par.prob_ls
            ...
            [bestx,bestold,current_eval,succ] = LS2 (CostFunction, bestx,bestold,Par,current_eval,Par.Max_FES,Par.xmin,Par.xmax);
...
BestNetWork = ConsNet_Fcn(net,bestx);
GlobalCost=-bestold;
fprintf('EBOwithCMAR, NumNeuron: %d, run:%d, bestR2:%3.5f \n',hiddenLayerSize,runn,GlobalCost);
end
...
function [x, xold, fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq,current_eval,res_det ] = ...
    EBO(CostFunction, x,xold, fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq, xmin, xmax,  n,...
...
for brojac = 1:size(ui,1)
    fitx_new(brojac) = CostFunction(ui(brojac,:));
end
...
% =========================================================================
function [x,f,current_eval,succ] = LS2 (CostFunction, bestx,f,Par,current_eval,Max_FES,xmin,xmax)
...
[Xsqp, FUN , ~ , details]=fmincon(CostFunction, bestx(1,:),[],[],[],[],xmin,xmax, [],options);
...
function[ x, fitx, setting,bestold,bestx,bnd,fitness,current_eval,res_det] = ...
    Scout(CostFunction, x, ~, prob, setting, iter,bestold,bestx,fitness,bnd,xmin,xmax,n,PopSize,current_eval,I_fno,res_det,Printing,Max_FES, constraints)
...
for brojac = 1:size(arxvalid,2)
    fitness.raw(brojac) = CostFunction(arxvalid(:,brojac)');
end
